...
#     def __init__(self, input_dim=10, hidden_dim=32, output_dim=1, dropout=0.1):
...
#     # print(param)
#     zero_param += torch.sum(torch.abs(param.grad) <= 1e-8)
#     total_param += param.grad.numel()
# print("Sparsity ratio:", zero_param/total_param)
model = ResNet18()
x = torch.randn(1, 3, 64, 64)  # batch of 5 samples, each with 10 features
# print(model)
output = model(x)
