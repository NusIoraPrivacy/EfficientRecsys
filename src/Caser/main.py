from Caser.utils import *
from Caser.models import Caser
from Caser.models_colr import Caser_CoLR
...
if __name__ == '__main__':
    ...
    model = Caser(n_users, n_items, args).to(args.device)
    ...
    if args.compress == "colr":
        model = Caser_CoLR(n_users, n_items, args).to(args.device)
    ...
    with tqdm(total=total_rounds) as pbar:
        for epoch in range(args.num_epochs):
            ...
            for batch in train_loader: # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
                ...
                # print(batch)
                u = u.to(args.device).squeeze()
                seq = seq.to(args.device)
                pos = pos.to(args.device)
                neg = neg.to(args.device)
                items_to_predict = torch.cat((pos, neg), 1)
                # print(u.shape)
                # print(seq.shape)
                # print(items_to_predict.shape)
                items_prediction = model(u, seq, items_to_predict)
                # print(items_prediction.shape)
                (pos_logits, neg_logits) = torch.split(items_prediction,
                                                     [pos.size(1),
                                                      neg.size(1)], dim=1)
