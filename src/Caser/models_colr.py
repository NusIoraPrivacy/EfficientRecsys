...
from Caser.models import activation_getter
import math

class Caser_CoLR(nn.Module):
    """
    ...
    def __init__(self, num_users, num_items, model_args):
        super(Caser_CoLR, self).__init__()
        ...
        self.item_embeddings = nn.Embedding(num_items+1, dims).requires_grad_(False)
        self.A = torch.nn.Parameter(torch.zeros(num_items+1, self.args.rank))
        self.B = torch.zeros(self.args.rank, dims).to(self.args.device)
    ...
    def update_params(self):
        self.item_embeddings.weight.data = self.A @ self.B + self.item_embeddings.weight
        torch.nn.init.normal_(self.B, mean=0, std=1*math.sqrt(1/self.args.rank))
    ...
    def forward(self, user_var, seq_var, item_var, for_pred=False):
        ...
        if for_pred:
            ...
            item_emb_mat = self.A @ self.B + self.item_embeddings.weight
            item_embs = item_emb_mat[seq_var].unsqueeze(1)
        ...
        if self.n_v:
            ...
            # out_v = out_v.view(self.args.batch_size, -1)  # prepare for fully connect
