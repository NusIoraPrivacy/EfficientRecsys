...
from utils.globals import private_param_dict, models_w_feats, colr_param_dict, item_param_dict
...
def train_fl_model(user_id_list, item_id_list, train_dataset, test_dataset, model, args):
    ...
    colr_params = colr_param_dict[args.model]
        ...
        for epoch in range(args.epochs):
            ...
            for this_user_list in uid_seq:
                ...
                for name, param in model.named_parameters():
                    if name not in private_params+item_params:
                        public_agg[name] = 0
                n_train_users = 0
                for i in this_user_list:
                    ...
                    if args.model in models_w_feats:
                        user_feat = user_feat.to(args.device)
                        item_feat = item_feat.to(args.device)
                    predictions = model.forward(this_users, this_items, user_feats=user_feat, item_feats=item_feat)
